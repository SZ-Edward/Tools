...
log_dir = "logs"
log_name = "[REQUEST_API]"

setup_logging(SentryHandler(config.raven_client,level=logging.ERROR))
...
class Logger(object):
    ...
    def _initLogger(self):
        ...
        # debug 单独放到debug文件
        #filehandler = MyHandler(self.log_dir, "%s.debug" % self.log_name)
        #filehandler.suffix = "%Y%m%d.log"
...
class Log():
    def initLog(self,logfile='.' + os.path.sep + log_dir + os.path.sep + time.strftime('%Y-%m-%d', time.localtime())+'.log',
